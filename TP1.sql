...
FROM
(
SELECT TEMPS.ANNEE, PRODUITS.CATEGORY, PRODUITS.PNAME, RANK () OVER (PARTITION BY TEMPS.ANNEE, PRODUITS.CATEGORY ORDER BY sum(QTE*PU) DESC) RANG
FROM VENTES
JOIN PRODUITS ON VENTES.PID=PRODUITS.PID 
JOIN TEMPS ON VENTES.TID=TEMPS.TID
GROUP BY TEMPS.ANNEE, PRODUITS.CATEGORY, PRODUITS.PNAME
ORDER BY TEMPS.ANNEE, PRODUITS.CATEGORY
)
WHERE RANG=1;
